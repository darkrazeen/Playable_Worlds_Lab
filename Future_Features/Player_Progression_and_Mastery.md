...
- **It unifies what's already scattered.** Faction standing, NPC arcs, gear tiers, quest rewards, and ledger flags each express a _slice_ of progression. This gives them one coherent home and a shared vocabulary.
...
_Place after Phase 3 consequence engine work (session-local), with persistence after Phase 6; exact week IDs assigned at approval time._
...
| [Faction & Reputation](./Faction_and_Reputation_System.md) | Standing is _relationship_ state; a milestone may be _earned from_ standing, not the same store |
...
4. Should gear unlocks _be_ progression unlocks, or stay a separate (linked) namespace?

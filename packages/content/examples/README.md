...
JSON fixtures for schema and validator tests. **Not** the canonical production world (see `../worlds/stonepass/`).

| File | Validates |
| --- | --- |
| `world-dna-teen.example.json`, `world-dna-adult.example.json` | WorldDNA |
| `player-choice-valid.example.json` | PlayerChoice |
| `story-beat-ogre-bridge.example.json` | StoryBeat |
...
| `world-definition-stonepass-minimal.example.json` | WorldDefinition + `validateWorldDefinition` (composition demo) |
| `world-definition-stonepass-invalid.example.json` | Cross-file validation failure |
| `world-session-stonepass-start.example.json` | WorldSession |
| `debug-event-*.example.json` | DebugEvent |
| `ai-result-*.example.json` | AIResult |
